...
{
    // Variables
    std::string foods_file_name = "../data/foods.csv"; 
    std::string daily_progress_file_name = "../data/daily_progress.csv"; 
    double weight = 0.0;
    ...
    database->load_from_file(foods_file_name);
    ...
    // Save data to files
    database->save_to_file(foods_file_name);
    today_log->save_daily_progress(weight, daily_progress_file_name);

    // Call Python script to generate prediction for next day
    system("../venv/bin/python ../ai/predict.py ../data/daily_progress.csv ../data/prediction.txt");

    // Read prediction from file and display to user
    std::ifstream pred_file("../data/prediction.txt");
    
    std::string prediction;
    if(pred_file.is_open()) 
    {
        while(getline(pred_file, prediction)) 
        {
            std::cout << "\n" << prediction << std::endl;
        }
        pred_file.close();
    ...
    // Call Python script for advice
    std::string python_command = "../venv/bin/python ../ai/coach.py " + goal;
    system(python_command.c_str());

    // Read advice from file and display to user
    std::ifstream advice_file("../data/advice.txt");
    std::string advice;
    if(advice_file.is_open()) 
    {
        while(getline(advice_file, advice)) 
        {
            std::cout << "\n" << advice << std::endl;
        }
    advice_file.close();
