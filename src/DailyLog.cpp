...
void Daily_log::save_daily_progress(double weight, std::string filename)
{
    std::ofstream progress_file;
    progress_file.open(filename, std::ios::app);

    if(!progress_file.is_open())
    {
        std::cerr << "Error: Could not open daily_progress.csv for writing." << std::endl;
        return;
    }
    else
    {
        progress_file << "\n" << date << "," << weight << "," << get_total_calories() << "," 
                    << get_total_protein() << "," << get_total_carbs() << "," 
                    << get_total_fats();

        std::cout << "\nSuccessfully saved daily log data to file!" << std::endl;
        progress_file.close();
    }
}